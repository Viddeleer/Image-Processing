	...
	for (dy = from; dy < to; ++dy)
	{
		...
		for (int dx = 0; dx < dst_w; ++dx)
		{
			...
			for (int ky = -radius_y; ky <= radius_y; ++ky)
			{
				...
				if (lut_y >= MAGIC_KERNEL_LUT_SIZE_2021_FAST) continue;

				// get interpolated LUT value
				frac_yi = dyf & MAGIC_KERNEL_FAST_PRECISION_MASK;							
				wy0 = magic_kernel_lut_2021_INT_FAST[lut_y];
				wy1 = magic_kernel_lut_2021_INT_FAST[lut_y + 1];
				...
				//wy = magic_kernel_lut_2021_INT_FAST[lut_y];
				...
				for (int kx = -radius_x; kx <= radius_x; ++kx)
				{
					...
					if (lut_x >= MAGIC_KERNEL_LUT_SIZE_2021_FAST) continue;

					// get interpolated LUT value
					frac_xi = dxf & MAGIC_KERNEL_FAST_PRECISION_MASK;
					wx0 = magic_kernel_lut_2021_INT_FAST[lut_x];
					wx1 = magic_kernel_lut_2021_INT_FAST[lut_x + 1];
					...
					// fastest LUT alternative without interpolation, but with small rounding errors
					//wx = magic_kernel_lut_2021_INT_FAST[lut_x];
...
HANDLE threadhandles[MAXTHREADS];
	...
	init_magic_kernel_lut_2021_INT_FAST();
