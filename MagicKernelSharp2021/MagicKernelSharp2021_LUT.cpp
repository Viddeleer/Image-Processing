...
static inline double srgb_to_linear(double s)  // s in [0,1]
{
	if (s <= 0.04045) return s / 12.92;
	return pow((s + 0.055) / 1.055, 2.4);
}

static inline double linear_to_srgb(double v)  // v in [0,1]
{
	if (v <= 0.0031308) return v * 12.92;
	return 1.055 * pow(v, 1.0 / 2.4) - 0.055;
}

inline void initialize_MKS2021_LUT()
{
...
void WINAPI ResizeImageMagicKernelSharp2021Thread_LUT(LPVOID lpParameters)
{
	float* dst;
	float* src;
	...
	for (int dy = from; dy < to; ++dy)
	{
		...
		for (int dx = 0; dx < dst_w; ++dx)
		{
			...
			for (int ky = -radius_y; ky <= radius_y; ++ky)
			{
				...
				double wy = MKS_2021_LUT[(int)(0.5 + dy_dist * MKS2021_LUT_RES)];
				...
				for (int kx = -radius_x; kx <= radius_x; ++kx)
				{
					...
					double wx = MKS_2021_LUT[(int)(0.5 + dx_dist * MKS2021_LUT_RES)];
					...
					if (src_c >= 4) sum_a += src[pixel_index] * w;		// may be removed when only handling RGB images
					wsum += w;
				}
			}
			...
			if (wsum > 0.0)
			{
				...
				if (dst_c >= 4) dst[idx] = (float)(sum_a * wsum);		// may be removed when only handling RGB images
			}
			else
			{
				//if (wsum < 0.0) MessageBox(NULL, "below zero", "debug", MB_OK);
				// no contributors
				dst[idx++] = dst[idx++] = dst[idx++] = 0;
				if (dst_c >= 4) dst[idx] = 0;							// may be removed when only handling RGB images
			}
		}
	}
}

unsigned char* ResizeImageMagicKernelSharp2021MultiThreaded_LUT(unsigned char* src, int src_w, int src_h, int src_c, int dst_w, int dst_h, int dst_c, int nrthreads)
{
	int curthread = 0;
	unsigned char* dst;
	float* tmpin, * tmpout;
	...
	initialize_MKS2021_LUT();
	
	// populate the temporary float input buffer, perform colorspace conversion from sRGB to linear RGB	
	for (i = 0; i < insize; i++) tmpin[i] = srgb_to_linear_lut[src[i]];
	...
	if (nrthreads == 1)	// Single thread
	{
		...
		ResizeImageMagicKernelSharp2021Thread_LUT((LPVOID)&ThreadParameters);
	}
	else				// Multithreaded version
	{
		for (curthread = 0; curthread < nrthreads; curthread++)
		{
			...
			do
			{
				threadhandles[curthread] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ResizeImageMagicKernelSharp2021Thread_LUT, (LPVOID)&ThreadParameters[curthread], 0, NULL);
	...
	// populate the final output buffer, perform colorspace conversion from linear to sRGB	
	float fval;
	for (i = 0; i < outsize; i++)
	{
		fval = 255.0 * linear_to_srgb(tmpout[i]);
