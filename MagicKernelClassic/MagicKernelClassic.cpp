...
#define MAGIC_KERNEL_RADIUS_CLASSIC 		2
...
#define MAGIC_KERNEL_LUT_SIZE_CLASSIC_FAST	((int)(MAGIC_KERNEL_RADIUS_CLASSIC * MAGIC_KERNEL_LUT_RES_FAST + 1))	// lookup table size

static int   magic_kernel_lut_classic_INT_FAST[MAGIC_KERNEL_LUT_SIZE_CLASSIC_FAST];

int MKCLASSIC_INITIALIZED = 0;

// Calculate the kernel
static inline float magic_kernel_classic(float x) // magic_kernel_official_implementation, , uses MAGIC_KERNEL_RADIUS = 2
{
	if (x < 0.0f) x = -x;
...
static void init_magic_kernel_lut_classic_INT_FAST()
{
	if (MKCLASSIC_INITIALIZED) return;
	for (int i = 0; i < MAGIC_KERNEL_LUT_SIZE_CLASSIC_FAST; ++i) 
	{
		float x = (float)i / MAGIC_KERNEL_LUT_RES_FAST;
		magic_kernel_lut_classic_INT_FAST[i] = (int)round(MAGIC_KERNEL_FAST_PRECISION_FACTOR * magic_kernel_sharp_classic(x));						
	}
	MKCLASSIC_INITIALIZED = 1;
}
...
void WINAPI ResizeImageMagicKernelClassicThread(LPVOID lpParameters)
{
	...
	int radius_x = 1 + (( 3 * scale_x / MAGIC_KERNEL_RADIUS_CLASSIC) >> 16); // ceil
	int radius_y = 1 + (( 3 * scale_y / MAGIC_KERNEL_RADIUS_CLASSIC) >> 16); // ceil
	...
	for (dy = from; dy < to; ++dy)
	{
		...
		for (int dx = 0; dx < dst_w; ++dx)
		{
			...
			for (int ky = -radius_y; ky <= radius_y; ++ky)
			{
				...
				if (lut_y >= MAGIC_KERNEL_LUT_SIZE_CLASSIC_FAST) continue;

				// get interpolated LUT value
				frac_yi = dyf & MAGIC_KERNEL_FAST_PRECISION_MASK;							
				wy0 = magic_kernel_lut_classic_INT_FAST[lut_y];
				wy1 = magic_kernel_lut_classic_INT_FAST[lut_y + 1];
				...
				//wy = magic_kernel_lut_classic_INT_FAST[lut_y];
				...
				for (int kx = -radius_x; kx <= radius_x; ++kx)
				{
					...
					if (lut_x >= MAGIC_KERNEL_LUT_SIZE_CLASSIC_FAST) continue;

					// get interpolated LUT value
					frac_xi = dxf & MAGIC_KERNEL_FAST_PRECISION_MASK;
					wx0 = magic_kernel_lut_classic_INT_FAST[lut_x];
					wx1 = magic_kernel_lut_classic_INT_FAST[lut_x + 1];
					...
					// fastest LUT alternative without interpolation, but with small rounding errors
					//wx = magic_kernel_lut_classic_INT_FAST[lut_x];
...
unsigned char *ResizeImageMagicKernelClassicMultiThreaded(unsigned char *src, int src_w, int src_h, int src_c, int dst_w, int dst_h, int dst_c, int nrthreads)
{
int curthread = 0;
unsigned char *dst;
HANDLE threadhandles[MAXTHREADS];
	...
	init_magic_kernel_lut_classic_INT_FAST();
	...
	if (nrthreads == 1)	// Single thread
	{
		...
		ResizeImageMagicKernelClaasicThread((LPVOID)&ThreadParameters);
	}
	else				// Multithreaded version
	{
		for (curthread = 0; curthread < nrthreads; curthread++)
		{
			...
			do
			{
				threadhandles[curthread] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ResizeImageMagicKernelClaasicThread, (LPVOID)&ThreadParameters[curthread], 0, NULL);
