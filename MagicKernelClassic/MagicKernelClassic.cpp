...
void WINAPI ResizeImageMagicKernelClassicThread(LPVOID lpParameters)
{
	...
	if (dst_w < src_w) radius_x = (int)ceil(1.5 * scale_x);
	if (dst_h < src_h) radius_y = (int)ceil(1.5 * scale_y);
	...
	for (int dy = from; dy < to; ++dy)
	{
		...
		for (int dx = 0; dx < dst_w; ++dx)
		{
			...
			for (int ky = -radius_y; ky <= radius_y; ++ky)
			{
				...
				if (dy_dist >= 1.5) continue;
				double wy = magic_kernel_classic(dy_dist);
...
static inline double srgb_to_linear(float s)  // s in [0,1]
{
	if (s <= 0.04045) return s / 12.92;
	return pow((s + 0.055) / 1.055, 2.4);
}

static inline double linear_to_srgb(float v)  // v in [0,1]
{
...
unsigned char* ResizeImageMagicKernelClassicMultiThreaded(unsigned char* src, int src_w, int src_h, int src_c, int dst_w, int dst_h, int dst_c, int nrthreads)
{
	...
	for (i = 0; i < insize; i++) tmpin[i] = 255.0 * srgb_to_linear(tmpin[i] / 255.0);
	...
	else				// Multithreaded version
	{
		for (curthread = 0; curthread < nrthreads; curthread++)
		{
			...
			do
			{
				threadhandles[curthread] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ResizeImageMagicKernelClassicThread, (LPVOID)&ThreadParameters[curthread], 0, NULL);
	...
	// if necessary, perform colorspace conversion from linear back to sRGB
	for (i = 0; i < outsize; i++) tmpout[i] = 255.0 * linear_to_srgb(tmpout[i] / 255.0);
